...
3. **`instrumentation.ts` is not the ffmpeg gate.** Under `node .next/standalone/server.js`, Next 16 silently skips `register()` ([vercel/next.js#89377](https://github.com/vercel/next.js/issues/89377)). The authoritative check is `electron/ffmpeg-verify.cjs`, run in main before the child spawns. For the same reason, the ffmpeg **shutdown reaper** (SIGTERM/SIGINT → `killAllProcesses()`) is installed lazily in `src/server/ffmpeg/spawn.ts` on first render spawn — *not* in `register()` — so a packaged child still reaps in-flight renders instead of orphaning them when `server-manager.cjs` kills it.
